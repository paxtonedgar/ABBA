...
// The PropertyAmenityFunc type is an adapter to allow the use of ordinary
// function as PropertyAmenity mutator.
type PropertyAmenityFunc func(context.Context, *ent.PropertyAmenityMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PropertyAmenityFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PropertyAmenityMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PropertyAmenityMutation", m)
}
