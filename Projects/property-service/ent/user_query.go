...
type UserQuery struct {
	...
	withReviewedProperties *PropertyQuery
...
// QueryReviewedProperties chains the current query on the "reviewed_properties" edge.
func (_q *UserQuery) QueryReviewedProperties() *PropertyQuery {
	query := (&PropertyClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withReviewedProperties: _q.withReviewedProperties.Clone(),
...
// WithReviewedProperties tells the query-builder to eager-load the nodes that are connected to
// the "reviewed_properties" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithReviewedProperties(opts ...func(*PropertyQuery)) *UserQuery {
	query := (&PropertyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withReviewedProperties = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withReviewedProperties; query != nil {
		if err := _q.loadReviewedProperties(ctx, query, nodes, nil,
			func(n *User, e *Property) { n.Edges.ReviewedProperties = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadProperties(ctx context.Context, query *PropertyQuery, nodes []*User, init func(*User), assign func(*User, *Property)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(property.FieldOwnerID)
	}
...
func (_q *UserQuery) loadReviewedProperties(ctx context.Context, query *PropertyQuery, nodes []*User, init func(*User), assign func(*User, *Property)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(property.FieldReviewedBy)
	}
	query.Where(predicate.Property(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ReviewedPropertiesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
