#!/usr/bin/env python3
"""
MLB 2024 Viable Fix - Addresses All Major Concerns
Fixes: Catastrophic drawdown, no out-of-sample testing, missing market realities
"""
...
class MLBViableFix:
    """Viable MLB betting system with proper risk management."""
    ...
    def __init__(self):
        ...
        # BALANCED betting settings (fixes drawdown but still bets)
        self.betting_config = {
            'min_ev_threshold': 0.01,   # 1% expected value minimum
            'max_risk_per_bet': 0.03,   # 3% max risk per bet
            'kelly_fraction': 0.35,     # 35% Kelly fraction
            'rolling_window': 15,       # 15-game rolling window
            'min_confidence': 0.51,     # 51% minimum confidence
            'max_bets_per_day': 4,      # 4 bets per day max
            ...
            'line_movement_impact': 0.01, # 1% line movement impact
            'bet_limit_factor': 0.8     # 80% of calculated bet size (account for limits)
        }

        logger.info("🚀 Initialized MLB Viable Fix with CONSERVATIVE settings")
    ...
    def create_viable_features(self, games_df: pd.DataFrame, standings_df: pd.DataFrame, stats_df: pd.DataFrame) -> pd.DataFrame:
        """Create features with proper data handling."""
        logger.info("🔧 Creating viable features")
    ...
    def viable_prediction_model(self, features_df: pd.DataFrame) -> pd.DataFrame:
        """Viable prediction model using realistic heuristics."""
        logger.info("🤖 Running viable prediction model")
        ...
        for idx, row in features_df.iterrows():
            ...
            form_adjustment = row['win_pct_diff'] * 0.25

            # Adjust for run differential
            run_adjustment = row['runs_diff'] * 0.015

            # Adjust for ERA differential
            era_adjustment = row['era_diff'] * 0.008
            ...
            # Simulate line movement impact
            line_movement = np.random.normal(0, self.betting_config['line_movement_impact'])
            home_prob += line_movement
            home_prob = max(0.35, min(0.65, home_prob))
    ...
    def conservative_betting_strategy(self, predictions_df: pd.DataFrame) -> pd.DataFrame:
        """Conservative betting strategy with proper risk management."""
        logger.info("💰 Running conservative betting strategy")
        ...
        for idx, row in predictions_df.iterrows():
            try:
                ...
                # Check max drawdown limit (only after we have bets)
                if len(bets) > 0:
                    current_drawdown = (running_max - current_bankroll) / running_max
                    if current_drawdown >= self.betting_config['max_drawdown_limit']:
                        continue
                ...
                if (ev >= self.betting_config['min_ev_threshold'] and
                    ...
                    # Apply bet limit factor (account for sportsbook limits)
                    bet_amount *= self.betting_config['bet_limit_factor']

                    # Conservative minimum bet size
                    if bet_amount >= 25:  # $25 minimum bet
                        ...
                        # Update bankroll and drawdown tracking
                        if row['home_win']:
    ...
    def run_validation(self):
        ...
        logger.info("🎯 Starting MLB Viable Fix Validation")
        ...
        # Create features
        features_df = self.create_viable_features(games_df, standings_df, stats_df)
        if len(features_df) == 0:
            logger.error("❌ No features created")
            return

        # Make predictions
        predictions_df = self.viable_prediction_model(features_df)

        # Place bets
        bets_df = self.conservative_betting_strategy(predictions_df)
        ...
        with open(output_dir / "viable_fix_results.json", 'w') as f:
            json.dump(results, f, indent=2, default=str)

        if len(bets_df) > 0:
            bets_df.to_csv(output_dir / "viable_fix_bets.csv", index=False)
