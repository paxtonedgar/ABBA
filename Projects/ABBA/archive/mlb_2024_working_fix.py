#!/usr/bin/env python3
"""
MLB 2024 Working Fix - Realistic Betting Strategy
Handles missing data properly and uses realistic thresholds
"""
...
class MLBWorkingFix:
    """Working MLB betting system with realistic thresholds."""
    ...
    def __init__(self):
        ...
        # Realistic betting settings
        self.betting_config = {
            'min_ev_threshold': 0.02,  # 2% expected value minimum
            'max_risk_per_bet': 0.03,  # 3% max risk per bet
            'kelly_fraction': 0.20,    # 20% Kelly fraction
            'rolling_window': 15,      # 15-game rolling window
            'min_confidence': 0.53,    # 53% minimum confidence
            'max_bets_per_day': 3,     # 3 bets per day max
            'daily_loss_limit': 0.08,  # 8% daily loss limit
            'transaction_cost': 0.025  # 2.5% transaction cost
        }

        logger.info("🚀 Initialized MLB Working Fix with realistic settings")
    ...
    def create_working_features(self, games_df: pd.DataFrame, standings_df: pd.DataFrame, stats_df: pd.DataFrame) -> pd.DataFrame:
        """Create features that handle missing data properly."""
        logger.info("🔧 Creating working features")
        ...
        for idx, game in games_df.iterrows():
            try:
                ...
                # Get standings with error handling
                home_standing = standings_df[standings_df['team'] == home_team]
                away_standing = standings_df[standings_df['team'] == away_team]
    ...
    def working_prediction_model(self, features_df: pd.DataFrame) -> pd.DataFrame:
        """Working prediction model using realistic heuristics."""
        logger.info("🤖 Running working prediction model")
    ...
    def run_validation(self):
        ...
        logger.info("🎯 Starting MLB Working Fix Validation")
        ...
        # Create features
        features_df = self.create_working_features(games_df, standings_df, stats_df)
        if len(features_df) == 0:
            logger.error("❌ No features created")
            return

        # Make predictions
        predictions_df = self.working_prediction_model(features_df)
        ...
        with open(output_dir / "working_fix_results.json", 'w') as f:
            json.dump(results, f, indent=2, default=str)

        if len(bets_df) > 0:
            bets_df.to_csv(output_dir / "working_fix_bets.csv", index=False)
