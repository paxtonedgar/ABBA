#!/usr/bin/env python3
"""
MLB 2024 Simple Viable - Core Fixes Only
Fixes: Drawdown, risk management, realistic betting
"""
...
class MLBSimpleViable:
    """Simple viable MLB betting system."""
        ...
        for idx, game in games_df.iterrows():
            try:
                ...
                features = {
                    ...
                    'era_diff': away_stats_mean.get('era', 4.5) - home_stats_mean.get('era', 4.5),
                }
    ...
    def simple_betting_strategy(self, predictions_df: pd.DataFrame) -> pd.DataFrame:
        """Simple betting strategy with risk management."""
        logger.info("💰 Running simple betting strategy")
        ...
        for idx, row in predictions_df.iterrows():
            try:
                ...
                # Simple betting criteria
                ev = row['expected_value']
                confidence = row['confidence']

                # Debug logging
                if idx < 10:  # Log first 10 games
                    logger.info(f"Game {idx}: EV={ev:.4f}, Confidence={confidence:.4f}, Thresholds: EV>={self.betting_config['min_ev_threshold']}, Conf>={self.betting_config['min_confidence']}")
                ...
                if (ev >= self.betting_config['min_ev_threshold'] and
                    ...
                    # Reduce bet size if drawdown is high
                    if len(bets) > 0:
                        current_drawdown = (running_max - current_bankroll) / running_max
                        if current_drawdown > 0.10:  # Reduce bets if >10% drawdown
                            kelly_fraction *= 0.5
    ...
    def run_validation(self):
        ...
        logger.info("🎯 Starting MLB Simple Viable Validation")
        ...
        # Place bets
        bets_df = self.simple_betting_strategy(predictions_df)
        ...
        with open(output_dir / "simple_viable_results.json", 'w') as f:
            json.dump(results, f, indent=2, default=str)

        if len(bets_df) > 0:
            bets_df.to_csv(output_dir / "simple_viable_bets.csv", index=False)
