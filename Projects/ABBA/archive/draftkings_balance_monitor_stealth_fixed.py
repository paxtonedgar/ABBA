"""
DraftKings Balance Monitor with BrowserBase Stealth Mode (Fixed)
Uses BrowserBase's built-in stealth capabilities with correct API parameters
"""
...
class DraftKingsStealthMonitor:
    ...
    async def create_stealth_session(self) -> bool:
        ...
        try:
            ...
            # Create session with stealth mode enabled using correct parameters
            self.session = bb.sessions.create(
                project_id=self.project_id,
                # Use stealth mode with proper parameters
                stealth=True,
            ...
            # Try with minimal stealth parameters
            try:
                self.logger.info("Trying with minimal stealth parameters...")
                bb = Browserbase(api_key=self.api_key)
                self.session = bb.sessions.create(
                    project_id=self.project_id,
                    stealth=True,
                ...
                self.logger.info(f"✅ Basic stealth session created: {self.session.id}")
                return True
            except Exception as e2:
                self.logger.error(f"❌ Failed to create basic stealth session: {e2}")
                return False
    ...
    async def monitor_balance(self) -> str:
        ...
        try:
            self.logger.info("🧪 DraftKings Balance Monitor - Stealth Mode (Fixed)")
            self.logger.info("=" * 50)
...
def main():
    """Main function."""
    print("🧪 DraftKings Balance Monitor - Stealth Mode (Fixed)")
    print("=" * 50)
